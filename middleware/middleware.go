...
// MustRegisterRetryMetrics registers the totalRetries metric with the provided prometheus.Registerer.
// It is a convenience function that ensures the metric is registered successfully, and panics if registration fails.
...
// The function creates a RetryMiddleware struct with the given parameters and returns it as a http.Handler.
...
func (r RetryMiddleware) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	for i := 0; ; i++ {
		...
		totalRetries.Inc()
		// exponential backoff
		jitter := time.Duration(rand.Intn(50)) //nolint:gosec
...
func (r *discardableResponseWriterWithReaderFrom) ReadFrom(re io.Reader) (int64, error) {
	...
	if r.immediateWrite {
		return r.ResponseWriter.(io.ReaderFrom).ReadFrom(re) //nolint: forcetypeassert
	}
...
func (m *lazyBodyCapturerWriteTo) WriteTo(w io.Writer) (int64, error) {
	if m.allRead {
		return m.reader.(io.WriterTo).WriteTo(w) //nolint: forcetypeassert
	}
